...
def state_update(t, x, u, params):
    ...
    a=1-0.1*abs(np.sin(2.5*t* np.pi))-0.1*abs(np.sin(batch_num * np.pi / 10))
...
#ax.view_init(0, 90)
if save_figure==True:
    plt.savefig('constrain_2000_n2000_30%_u1.png',dpi=700)
#plt.show()
#pdb.set_trace()
...
#ax.view_init(0, 90)
if save_figure==True:
    plt.savefig('constrain_2000_n2000_30%_u2.png',dpi=700)
plt.show()
#pdb.set_trace()
...
if save_csv==True:
    ...
    with open('./constrain_2000_n2000_30%_y1.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
    ...
    with open('./constrain_2000_n2000_30%_y2.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)

        # write multiple rows

        writer.writerows(map(lambda x: [x], SAE_y2))
    with open('./constrain_2000_n2000_30%_sum.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
...
if save_figure==True:
    plt.savefig('constrain_2000_n2000_30%_RMSE.png',dpi=700)
...
if save_figure==True:
    plt.savefig('constrain_2000_n2000_total_30%_RMSE.png',dpi=700)
