...
"5. Simulation of the linear CSTR system"

X0 = [1.0, 395.33]                 # Initial x1, x2
T = np.linspace(0, 3, 301)
t_con_linear, y_con_linear = control.input_output_response(Lin_CSTR, T, 0., X0,method='LSODA')

"6. Simulation of the discrete linear CSTR system"

X0 = [1.0, 395.33]                 # Initial x1, x2
T = np.linspace(0, 3, 301)
t_con_dis, y_con_dis = control.input_output_response(Dis_Lin_CSTR, T, 0., X0)
...
plt.plot(t_con_dis, y_con_dis,linewidth=1,linestyle='dashed')
#plt.legend(['Continous', 'Discrete T=1', 'Discrete T=0.5', 'Discrete T=0.1'])
plt.legend(['Nonlinear', 'Linear','Discrete'])
...
plt.savefig('CSTR_Discretization.png',dpi=700)
