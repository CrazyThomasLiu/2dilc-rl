...
import os
import control
from ilcenv_ilcinrl import BatchSysEnvq
...
def state_update(t, x, u, params):
    ...
    # Parameter setup
    # pdb.set_trace()
    sigma1 = 0.5 * np.sin(batch_num * 2 * np.pi / 5)
    sigma2 = 0.5 * np.sin(batch_num * 2 * np.pi / 5)
    sigma3 = 0.5 * np.sin(batch_num * 2 * np.pi / 5)
    sigma4 = 0.5 * np.sin(batch_num * 2 * np.pi / 5)
...
ILCsac_dir=os.path.join(current_dir, "bathsinuspi5.csv")
...
plt.savefig('batch_RMES_action10pi5.png',dpi=700)
plt.show()
