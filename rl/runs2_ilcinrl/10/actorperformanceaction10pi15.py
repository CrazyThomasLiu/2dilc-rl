...
def state_update(t, x, u, params):
    ...
    # Parameter setup
    # pdb.set_trace()
    sigma1 = 0.5 * np.sin(batch_num * 2 * np.pi / 15)
    sigma2 = 0.5 * np.sin(batch_num * 2 * np.pi / 15)
    sigma3 = 0.5 * np.sin(batch_num * 2 * np.pi / 15)
    sigma4 = 0.5 * np.sin(batch_num * 2 * np.pi / 15)
...
ILCsac_dir=os.path.join(current_dir, "bathsinuspi15.csv")
...
plt.plot(batch_time_show,y_ILC_show,linewidth=1.5,color='black',linestyle=':')
...
plt.savefig('batch_RMES_action10pi15.png',dpi=700)
plt.show()
