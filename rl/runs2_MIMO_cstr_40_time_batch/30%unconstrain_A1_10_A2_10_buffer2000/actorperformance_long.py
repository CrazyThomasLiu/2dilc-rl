...
from cstr_MIMO.env_MIMO_unconstrain_cstr_30_time_batch_u1_10_u2_2000 import BatchSysEnv
...
model_dir=os.path.join(current_dir, "actor_long.pth")
...
u_sum_batch = np.zeros((T_length,n))
u_sum_data = []
...
for batch_index in range(batch):
    for item in range(T_length):
        ...
        u_sum_batch[item, 0] = state[3]+state[0]
        u_sum_batch[item, 1] = state[17] + state[14]
...
"2.3 the sum of Control Signal"
"2.3.1 u1 Feed Concentration"
fig=plt.figure()
ax=plt.axes(projection="3d")
ax.invert_xaxis()
ax.xaxis.set_major_locator(MaxNLocator(integer=True))
...
for item2 in range(batch):
    batch_plot=Batch_length*(item2+1)
    if (item2%2)==0:
        ax.plot3D(batch_plot,t, u_sum_data[item2][:,0].squeeze(),linewidth=1,color='black')
...
ax.set_ylabel(ylable,font2)
ax.set_zlabel(zlable,font2)
plt.title('Input Signal: Sum Feed Concentration')
#ax.legend(['y_Ref','y_out'])
ax.view_init(52, -16)
ax.view_init(40, -19)
if save_figure==True:
    plt.savefig('the Sum of the Control Signal_u1.png',dpi=700)
#plt.show()
#pdb.set_trace()
...
for item2 in range(batch):
    batch_plot=Batch_length*(item2+1)
    if (item2%2)==0:
        ax.plot3D(batch_plot,t, u_sum_data[item2][:,1].squeeze(),linewidth=1,color='black')
...
ax.set_ylabel(ylable,font2)
ax.set_zlabel(zlable,font2)
plt.title('Input Signal: Sum Cooler')
#ax.legend(['y_Ref','y_out'])
ax.view_init(52, -16)
ax.view_init(40, -19)
if save_figure==True:
    plt.savefig('The sum of the Control Signal_u2.png',dpi=700)
plt.show()
pdb.set_trace()
"3.SAE"
font2 = {'family': 'Times New Roman',
...
ILCsac_dir=os.path.join(current_dir, "unconstrain_u1_10_u2_2000_30%_y1.csv")
...
with f_ILC:
    ...
    for row in reader:
        ...
        num+=1
ILCsac_dir=os.path.join(current_dir, "unconstrain_u1_10_u2_2000_30%_y2.csv")
f_ILC=open(ILCsac_dir,'r')
num=0
...
ILCsac_dir=os.path.join(current_dir, "unconstrain_u1_10_u2_2000_30%_sum.csv")
f_ILC=open(ILCsac_dir,'r')
num=0
...
if save_figure==True:
    plt.savefig('constrain_1000_n1000_total_RMSE.png',dpi=700)
