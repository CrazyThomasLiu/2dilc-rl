...
class BatchSysEnv:
    def __init__(self,T_length,sys,T=0.01,X0=np.array((0.47,396.9))):
        ...
        '30% robust'
        self.K = np.array([[-137.0651353, -50.59543044, 26.82552131, 26.77557816],
    ...
    def step(self,action):
        ...
        #pdb.set_trace()
        action_map_u1=8*action[0]
        #action_map_u2 = 40*action[1] -40
        action_map_u2 = 600 * action[1]
        ...
        elif self.input_signal[0,0] < -10.:
            self.input_signal[0,0] = -10.
        #elif self.input_signal[0,0] < 0.:
        #    self.input_signal[0,0] = 0.
        # input 2 is the cooling so only negative
        if self.input_signal[1,0] < -2000.:
            self.input_signal[1,0] = -2000.
        elif self.input_signal[1,0] > 2000.:
            self.input_signal[1,0] = 2000.
        ...
        # the state space
        #pdb.set_trace()
        # TODO: input so be not constniared
        # 0 to 2 for the uRL
        ...
        #reward = self.A1 * abs(self.y_ref[self.time, 0] - y_step[0, 1])**2 + self.A2 * abs(
        #    self.y_ref[self.time, 1] - y_step[1, 1])**2
        #pdb.set_trace()
    ...
    def cal_2DILCcontroller(self):
        ...
        if self.u_k[0, 0] > 10.:
            self.u_k[0, 0] = 10.
        elif self.u_k[0, 0] < -10.:
            self.u_k[0, 0] = -10.
        #elif self.u_k[0, 0] < 0.:
        #    self.u_k[0, 0] = 0.
        # input 2 is the cooling so only negative
        if self.u_k[1, 0] < -2000:
            self.u_k[1, 0] = -2000
        elif self.u_k[1, 0] > 2000:
            self.u_k[1, 0] =2000
