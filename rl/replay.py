...
class ReplayBuffer:
    ...
    def sample_batch(self, batch_size) -> tuple:
        ...
        if self.per_tree:
            ...
            return (r_m_a[:, 0:1],  # reward
                    ...
                    self.buf_state[indices + 1])# state or next_state
    ...
    def save_or_load_history(self, cwd, if_save, buffer_id=0):
        ...
        if if_save:
            ...
            buf_state_data_type = np.float32
            buf_state = np.empty((self.max_len, state_dim), dtype=buf_state_data_type)
            buf_other = np.empty((self.max_len, other_dim), dtype=np.float32)
            #pdb.set_trace()
    ...
    def save_or_load_history_implementation(self, cwd, if_save, buffer_id=0):
        save_path = f"{cwd}/replay_{buffer_id}.npz"
        if_load = None
