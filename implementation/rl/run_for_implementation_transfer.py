...
def train_and_evaluate(args, agent_id=0):
    ...
    while if_train:
        ...
        with torch.no_grad():
            ...
            trajectory,rmse = agent.explore_env(env, target_step)
            #pdb.set_trace()
            #rmse=control_performance_RMSE(trajectory)
            #rmse=np.mean(rmse)
            steps, r_exp = update_buffer(trajectory)
        #pdb.set_trace()
        ...
        with torch.no_grad():
            ...
            temp = evaluator.evaluate_and_save_injection_modeling_rmse(agent.act, steps, r_exp, rmse)
            #########################################################################
            #########################################################################
    ...
    evaluator.save_or_load_recoder(if_save=True)
def control_performance_RMSE(_trajectory,time_length=200):
    """
    The evaluation index (RMSE) for the control performance each batch

    :param _trajectory: type [[]]
    :param time_length: the length of the time index
    """
    rmse=[]
    sum=0.0
    error=0.0
    for trajectory in _trajectory:
        ten_state =[item[0] for item in trajectory]
        for item in range(len(ten_state)):
            error=(ten_state[item][6]-ten_state[item][9])**2
            sum+=error
